...
void Graph::bfs(Node& start) {
  start.setColor(GRAY, clock);
	...
	vector<Node*> queue = search_nodes;
	start.setRank(0);
	...
	while ( !queue.empty())
	{
		...
		n->getDiscoveryInformation(n_col, n_dt, n_ft, n_rk);
		...
		for (; it != adj.end(); it++)
		{
			...
			other->getDiscoveryInformation(color, disco_time, finish_time, bfs_rank); 
			
			if (color == WHITE)
			{
				clock++; 
				other->setColor(GRAY, clock);
				other->setRank(n_rk + 1); 
				queue.insert(queue.begin(), other); 	
			} 
		}	
	}
}
